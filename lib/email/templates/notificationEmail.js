...
function getAppUrl(explicitAppUrl) {
  const vercelUrl = normalizeAppUrl(
    process.env.VERCEL_PROJECT_PRODUCTION_URL || process.env.VERCEL_URL
  );
  const configuredUrl = normalizeAppUrl(
    explicitAppUrl || process.env.APP_URL || process.env.NEXT_PUBLIC_APP_URL
  );

  if (process.env.NODE_ENV === 'production' && configuredUrl.includes('localhost') && vercelUrl) {
    return vercelUrl;
  }

  return configuredUrl || vercelUrl || 'http://localhost:3000';
}
...
export function notificationEmail({
  ...
  const appUrl = getAppUrl(explicitAppUrl);
