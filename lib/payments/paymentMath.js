...
export function calculatePaymentPlan({
  ...
  return {
    ...
    downpaymentPercentage: toMoneyNumber(downpaymentPercentage),
    initialAmountDue,
    principalBalance: roundMoney(Math.max(0, totalContractPrice - initialAmountDue)),
    remainingBalance: totalContractPrice,
    installmentTermMonths: Math.max(1, Number.parseInt(installmentTermMonths, 10) || 6),
    monthlyPayment: roundMoney(Math.max(0, totalContractPrice - initialAmountDue) / Math.max(1, Number.parseInt(installmentTermMonths, 10) || 6)),
    interestRate: roundMoney(interestRate),
    status: 'pending_initial_payment'
  };
}
