...
export function getAmountDueForReservationStart({
  isAuthenticated,
  propertyPrice,
  reservationFee,
  paymentType,
  ...
  if (!isAuthenticated) return holdFee;

  if (paymentType === 'full_payment') {
    return roundMoney(fullPaymentAmount ?? propertyPrice);
  }

  const remainingEquity = remainingDownpayment !== undefined
    ? roundMoney(remainingDownpayment)
    : roundMoney(Math.max(0, toMoneyNumber(requiredDownpayment) - holdFee));

  return roundMoney(holdFee + remainingEquity);
}
