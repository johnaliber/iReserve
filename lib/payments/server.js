...
import { addCalendarMonths, dateOnly, normalizeMonthlyScheduleRows } from './scheduleDates';
...
export async function createPaymentPlanForReservation(admin, {
  ...
  const hasMonthlySchedule = plan.paymentType !== 'full_payment' && plan.installmentTermMonths && plan.monthlyPayment;
  const nextDueDate = hasMonthlySchedule ? addCalendarMonths(startDate, 1) : null;
  ...
  const { data: paymentPlan, error: planError } = await admin
    ...
    .insert({
      ...
      next_due_date: nextDueDate ? dateOnly(nextDueDate) : null,
      status: plan.status,
  ...
  if (hasMonthlySchedule) {
    const rows = Array.from({ length: plan.installmentTermMonths }, (_, index) => {
      const dueDate = dateOnly(addCalendarMonths(startDate, index + 1));
      return {
        payment_plan_id: paymentPlan.id,
        reservation_id: reservationId,
        due_number: index + 1,
        due_date: dueDate,
        amount_due: index === plan.installmentTermMonths - 1
          ? plan.finalInstallmentPayment
          : plan.monthlyPayment,
        amount_paid: 0,
        remaining_due: index === plan.installmentTermMonths - 1
          ? plan.finalInstallmentPayment
          : plan.monthlyPayment,
        status: 'unpaid'
      };
...
export async function recalculatePaymentIndicators(admin, paymentPlanId) {
  ...
  const normalizedScheduleRows = normalizeMonthlyScheduleRows(scheduleRows || []);
  const amountPaid = roundMoney((verifiedPayments || []).reduce((sum, payment) => sum + Number(payment.amount || 0), 0));
  const scheduledBalance = roundMoney(
    normalizedScheduleRows.reduce((sum, row) => sum + Number(row.amount_due || 0), 0)
  );
  const totalPaymentDue = paymentPlan.payment_type === 'installment' && scheduledBalance > 0
    ? roundMoney(Number(paymentPlan.initial_amount_due || 0) + scheduledBalance)
    : Number(paymentPlan.total_contract_price || 0);
  const today = new Date().toISOString().slice(0, 10);
  const overdueRows = normalizedScheduleRows.filter((row) => row.due_date < today && ['unpaid', 'partially_paid'].includes(row.status));
  const nextDue = normalizedScheduleRows.find((row) => ['unpaid', 'partially_paid', 'overdue'].includes(row.status));
...
export async function createFallbackPaymentSchedule(admin, paymentPlanId) {
  ...
  const { data: existingRows, count } = await admin
    .from('payment_schedule')
    .select('*', { count: 'exact' })
    .eq('payment_plan_id', paymentPlanId)
    .order('due_number', { ascending: true });

  const term = Math.max(1, Number(plan.installment_term_months || 6));
  const rows = normalizeMonthlyScheduleRows(existingRows || []);
  const latestRow = rows.at(-1);
  const amountDue = Number(plan.monthly_payment || 0) > 0
    ? Number(plan.monthly_payment)
    : roundMoney(Number(plan.remaining_balance || 0) / term);
  const dueDate = latestRow?.due_date
    ? dateOnly(addCalendarMonths(latestRow.due_date, 1))
    : plan.next_due_date || dateOnly(addCalendarMonths(plan.start_date || plan.created_at || new Date(), 1));
  ...
  if (error || !schedule) throw new Error(error?.message || 'Payment due could not be created.');
  return schedule;
}

export async function ensureNextPaymentSchedule(admin, paymentPlanId, paidSchedule) {
  if (!paidSchedule || Number(paidSchedule.remaining_due || 0) > 0) return null;
  ...
  if (!plan || Number(plan.remaining_balance || 0) <= 0) return null;
  ...
  const rows = normalizeMonthlyScheduleRows(scheduleRows || []);
  const openRow = rows.find((row) => ['unpaid', 'partially_paid', 'overdue'].includes(row.status));
  if (openRow || rows.length >= term) return null;

  const nextDueNumber = Math.max(...rows.map((row) => Number(row.due_number || 0)), Number(paidSchedule.due_number || 0)) + 1;
  const normalizedPaidSchedule = rows.find((row) => row.id === paidSchedule.id) || paidSchedule;
  const amountDue = Number(plan.monthly_payment || 0) > 0
    ? Number(plan.monthly_payment)
    : roundMoney(Number(plan.remaining_balance || 0) / Math.max(1, term - rows.length));
  const dueDate = dateOnly(addCalendarMonths(normalizedPaidSchedule.due_date || plan.next_due_date || new Date(), 1));
