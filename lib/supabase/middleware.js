...
export const updateSession = async (request) => {
  ...
  if (user) {
    ...
    const isSharedLedgerRoute = path.startsWith('/accounting/ledger/');
    const canAccessAccountingRoute =
      role === 'accounting' ||
      role === 'super_admin' ||
      (role === 'village_admin' && isSharedLedgerRoute);

    if (path.startsWith('/accounting') && !canAccessAccountingRoute) {
      url.pathname = '/';
      return NextResponse.redirect(url);
    }
