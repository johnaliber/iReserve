      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p id="confirm-dialog-description" className="mt-2 text-sm leading-6 text-[#64748b]">{message}</p>
          </div>
        ...
        <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          <button type="button" onClick={onCancel} disabled={busy} className="min-h-11 rounded-xl border border-[#dbe4ee] bg-white px-4 text-sm font-bold text-[#272727] hover:bg-[#f8fafc] disabled:opacity-60">
            {cancelLabel}
          </button>
          <button ref={confirmRef} type="button" onClick={onConfirm} disabled={busy} className={`min-h-11 rounded-xl px-4 text-sm font-extrabold text-white disabled:opacity-60 ${destructive ? 'bg-rose-600 hover:bg-rose-500' : 'bg-emerald-600 hover:bg-emerald-500'}`}>
