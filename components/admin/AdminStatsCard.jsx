    ...
    <div className="rounded-2xl border border-[#e2e8f0] bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1.5 text-2xl font-extrabold leading-none text-[#272727]">{value}</p>
          {helper && <p className="mt-1 text-xs text-[#64748b]">{helper}</p>}
        </div>
        {Icon && <div className={`rounded-xl p-2 ${tones[tone] || tones.emerald}`}><Icon className="h-4.5 w-4.5" /></div>}
      </div>
    </div>
  );
}
